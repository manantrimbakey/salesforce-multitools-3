...
export function registerComponentFileSwitcherCommands(context: vscode.ExtensionContext): void {
    ...
    context.subscriptions.push(
        switchComponentFileCmd,
        refreshComponentDataCmd,
        {
            dispose: () => {
...
async function handleSwitchComponentFile(): Promise<void> {
    ...
    try {
        ...
        const selectedFile = otherFiles.find(file => 
            selection.label.endsWith(file.name) && selection.description === file.type
        );
...
function sendComponentDataMessage(data: {
    ...
    vscode.commands.executeCommand('setContext', 'salesforceMultitools.isLightningComponent', 
